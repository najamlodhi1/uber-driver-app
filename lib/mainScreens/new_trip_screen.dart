...
class _NewTripScreenState extends State<NewTripScreen>
{
  ...
  Future<void> drawPolyLineFromOriginToDestination(LatLng originLatLng, LatLng destinationLatLng) async
  {
    showDialog(
      context: context,
      builder: (BuildContext context) => ProgressDialog(message: Locales.string(context, "pleasewait"),),
    );
  ...
  createDriverIconMarker()
  {
    if(iconAnimatedMarker == null)
    {
      ...
      BitmapDescriptor.fromAssetImage(imageConfiguration, "assets/images/mapCar.png").then((value)
      {
  ...
  endTripNow() async
  {
    ...
    late double totalFareAmount;

    DatabaseEvent event = await FirebaseDatabase.instance.ref().child("All Ride Requests")
        .child(widget.userRideRequestDetails!.rideRequestId!)
        .child("price").once();
    print(event.snapshot.value.toString()+" is the price of user to driver");
    if(event.snapshot.value != null){
      totalFareAmount = double.parse(event.snapshot.value.toString());
    }
  ...
  saveAssignedDriverDetailsToUserRideRequest()
  {
    ...
    databaseReference.child("car_details").set(onlineDriverData.car_color.toString() +" "+ onlineDriverData.car_model.toString()+" "+ onlineDriverData.car_number!);
